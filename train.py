...
def train(config):
    ...
    scheduler = MultiStepLR(optimizer, milestones=milestones, gamma=config.lr_gamma)
    
    # Set up loss function
    criterion = nn.CrossEntropyLoss(weight=torch.FloatTensor([0.05, 1.0]).to(device))
    ...
    for epoch in range(config.epochs):
        ...
        for batch_idx, sample_batched in enumerate(pbar):
            ...
            align_loss_meter.update(align_loss.item() if isinstance(align_loss, torch.Tensor) else align_loss)
            ...
            if i_iter % config.val_interval == 0:
                ...
                print(f"Iteration {i_iter} | Validation Dice: {dice_score:.4f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train ALPNet for fundus vessel segmentation")
